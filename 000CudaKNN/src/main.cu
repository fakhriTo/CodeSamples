...
const int N_POINTS = 1e7, N_QUERIES = 1e6, INF = 1e9, RANGE_MAX = 100, N_PRINT = 10;
...
int main() {
    ...
    cudaMallocManaged(&points, N_POINTS * sizeof(int3));
    cudaMallocManaged(&tree, TREE_SIZE * sizeof(int3));
    cudaMallocManaged(&queries, N_QUERIES * sizeof(int3));
    ...
    nearestNeighborGPU<<</*32768*/1024, 32>>>(tree, TREE_SIZE, queries, results, N_QUERIES);
